...
class Database {
    ...
    private function __construct() {
        $this->connection = new mysqli("localhost", "root", "", "abc_lab");
...
class PatientRegistrar {
    ...
    public function registerPatient($data) {
        ...
        $stmt->bind_param("sssssss", $data['fname'], $data['lname'], $data['gender'], $data['email'], $data['contact'], $data['password'], $data['cpassword']);
        $result = $stmt->execute();
...
// Class for managing appointment updates
...
// Class for managing doctor registration
class DoctorRegistrar {
    ...
    public function registerDoctor($name) {
        $connection = $this->db->getConnection();
...
if (isset($_POST['patsub1'])) {
    $patientRegistrar = new PatientRegistrar();
    $patientRegistrar->registerPatient($_POST);
}

if (isset($_POST['update_data'])) {
    $appointmentUpdater = new AppointmentUpdater();
    $appointmentUpdater->updateAppointmentStatus($_POST['contact'], $_POST['status']);
}

if (isset($_POST['doc_sub'])) {
    $doctorRegistrar = new DoctorRegistrar();
    $doctorRegistrar->registerDoctor($_POST['name']);
}
